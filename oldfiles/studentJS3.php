...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Sıralama başlıkları için olay dinleyicilerini ayarla
    let headers = document.querySelectorAll('.sortable');
    headers.forEach(header => {
        header.addEventListener('click', function () {
            let currentSort = header.getAttribute('data-sort');
            let sortOrder = header.getAttribute('data-order') === 'ASC' ? 'DESC' : 'ASC';
            header.setAttribute('data-order', sortOrder);
            getSortedData(currentSort, sortOrder);
        });
    });

    // Tablo başlığında tıklama olayını dinle
    document.querySelector('.table thead').addEventListener('click', resetEditing);
});
...
function yeni() {
    ...
    document.querySelectorAll("tr[data-editing='true']").forEach(row => {
        ...
        let sidOther = row.getAttribute("data-id");
...
document.addEventListener('click', function(e) {
    // Sakla butonuna basılmadıysa ve diğer butonlara basıldığında 
    // veya sıralama değiştirildiğinde Sakla'nın tekrar Güncelle'ye dönüşmesi için:
    if (e.target && (e.target.innerText === "Sil" || e.target.innerText === "Oluştur" || e.target.innerText === "Ara" || e.target.innerText === "Güncelle")) {
        resetEditing();
    }
    

    // Tablo gövdesindeki butonlar için olay dinleyicilerini kontrol ediyor:
    if (e.target && e.target.nodeName === "BUTTON" && e.target.closest('.table tbody')) {
...
function resetEditing() {
    document.querySelectorAll("tr[data-editing='true']").forEach(row => {
        ...
        let sid = row.getAttribute('data-id');
        let updateBtn = row.querySelector("button[onclick^='sakla']");
        if (updateBtn) {
            updateBtn.innerText = "Güncelle";
            updateBtn.setAttribute("onclick", `guncelle(${sid})`);
        }
    });
}
...
function getSortedData(sortType, sortOrder) {
    ...
    // Tüm sıralama başlıklarından 'data-order' özelliğini kaldır
    let headers = document.querySelectorAll('.sortable');
    headers.forEach(header => {
        header.removeAttribute('data-order'); 
    });

    // Şu anki sıralama başlığına 'data-order' özelliği ekleyerek sıralama yönünü belirt
    let currentHeader = document.querySelector(`.sortable[data-sort="${sortType}"]`);
    currentHeader.setAttribute('data-order', sortOrder); 
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            updatePaginationLinks(data.totalPages, currentPage); // Güncellenmiş sayfa numarasıyla paginasyon bağlantılarını güncelle
        }
    });
}
